...
class InvKinematics:
    ...
    def _init_transforms(self, params):
        ...
        self.T_base = (
            transl(base_xyz[0], base_xyz[1], base_xyz[2])
            @ trotx(base_rpy[0])
            @ troty(base_rpy[1])
            @ trotz(base_rpy[2])
        )
        self.T_tool = (
            transl(tool_xyz[0], tool_xyz[1], tool_xyz[2])
            @ trotx(tool_rpy[0])
            @ troty(tool_rpy[1])
            @ trotz(tool_rpy[2])
        )
    ...
    def solve_ik(self, x, y, z, roll, pitch, yaw, force, q0):
        ...
        if discriminante < 0 and den != 0:
            ...
            q_11 = (
                np.mod(q_11 - self.qlim1_inf, self.qlim1_sup - self.qlim1_inf)
                + self.qlim1_inf
            )
            q_12 = (
                np.mod(q_12 - self.qlim1_inf, self.qlim1_sup - self.qlim1_inf)
                + self.qlim1_inf
            )
        ...
        if s1_valid and s2_valid and s3_valid and s4_valid:
            ...
            q_611 = (
                np.mod(q_611 - self.qlim6_inf, self.qlim6_sup - self.qlim6_inf)
                + self.qlim6_inf
            )
            q_612 = (
                np.mod(q_611 - self.qlim6_inf + np.pi, self.qlim6_sup - self.qlim6_inf)
                + self.qlim6_inf
            )
        ...
        if s5_valid and s6_valid and s7_valid and s8_valid:
            ...
            q_621 = (
                np.mod(q_621 - self.qlim6_inf, self.qlim6_sup - self.qlim6_inf)
                + self.qlim6_inf
            )
            q_622 = (
                np.mod(q_621 - self.qlim6_inf + np.pi, self.qlim6_sup - self.qlim6_inf)
                + self.qlim6_inf
            )
        ...
        if s1_valid and s2_valid and s3_valid and s4_valid:
            ...
            num = np.cos(q_611) * (r_21 * np.cos(q_11) - r_11 * np.sin(q_11)) - np.sin(
                q_611
            ) * (r_22 * np.cos(q_11) - r_12 * np.sin(q_11))
            den = r_23 * np.cos(q_11) - r_13 * np.sin(q_11)

            q_511 = np.arctan2(num, den)
            q_511 = (
                np.mod(q_511 - self.qlim5_inf, self.qlim5_sup - self.qlim5_inf)
                + self.qlim5_inf
            )

            num = np.cos(q_612) * (r_21 * np.cos(q_11) - r_11 * np.sin(q_11)) - np.sin(
                q_612
            ) * (r_22 * np.cos(q_11) - r_12 * np.sin(q_11))
            den = r_23 * np.cos(q_11) - r_13 * np.sin(q_11)
            ...
            q_512 = (
                np.mod(q_512 - self.qlim5_inf, self.qlim5_sup - self.qlim5_inf)
                + self.qlim5_inf
            )

        if s5_valid and s6_valid and s7_valid and s8_valid:

            num = np.cos(q_621) * (r_21 * np.cos(q_12) - r_11 * np.sin(q_12)) - np.sin(
                q_621
            ) * (r_22 * np.cos(q_12) - r_12 * np.sin(q_12))
            den = r_23 * np.cos(q_12) - r_13 * np.sin(q_12)

            q_521 = np.arctan2(num, den)
            q_521 = (
                np.mod(q_521 - self.qlim5_inf, self.qlim5_sup - self.qlim5_inf)
                + self.qlim5_inf
            )

            num = np.cos(q_622) * (r_21 * np.cos(q_12) - r_11 * np.sin(q_12)) - np.sin(
                q_622
            ) * (r_22 * np.cos(q_12) - r_12 * np.sin(q_12))
            den = r_23 * np.cos(q_12) - r_13 * np.sin(q_12)
            ...
            q_522 = (
                np.mod(q_522 - self.qlim5_inf, self.qlim5_sup - self.qlim5_inf)
                + self.qlim5_inf
            )
        ...
        if s1_valid and s2_valid:
            ...
            if abs(np.sin(q_511)) < 1e-10:
                ...
                if discriminante < 0:
                    ...
                    q_21 = (
                        np.mod(q_21 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
                    q_22 = (
                        np.mod(q_22 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
        ...
        if s3_valid and s4_valid:
            ...
            if abs(np.sin(q_512)) < 1e-10:
                ...
                if discriminante < 0:
                    ...
                    q_23 = (
                        np.mod(q_23 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
                    q_24 = (
                        np.mod(q_24 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
        ...
        if s5_valid and s6_valid:
            ...
            if abs(np.sin(q_521)) < 1e-10:
                ...
                if discriminante < 0:
                    ...
                    q_25 = (
                        np.mod(q_25 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
                    q_26 = (
                        np.mod(q_26 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
        ...
        if s7_valid and s8_valid:
            ...
            if abs(np.sin(q_522)) < 1e-10:
                ...
                if discriminante < 0:
                    ...
                    q_27 = (
                        np.mod(q_27 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
                    q_28 = (
                        np.mod(q_28 - self.qlim2_inf, self.qlim2_sup - self.qlim2_inf)
                        + self.qlim2_inf
                    )
        ...
        if s1_valid:
            ...
            if abs(np.sin(q_511)) < 1e-10:
                ...
                q_31 = q23 - q_21
                q_31 = (
                    np.mod(q_31 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s2_valid:
            ...
            if abs(np.sin(q_511)) < 1e-10:
                ...
                q_32 = q23 - q_22
                q_32 = (
                    np.mod(q_32 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s3_valid:
            ...
            if abs(np.sin(q_512)) < 1e-10:
                ...
                q_33 = q23 - q_23
                q_33 = (
                    np.mod(q_33 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s4_valid:
            ...
            if abs(np.sin(q_512)) < 1e-10:
                ...
                q_34 = q23 - q_24
                q_34 = (
                    np.mod(q_34 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s5_valid:
            ...
            if abs(np.sin(q_521)) < 1e-10:
                ...
                q_35 = q23 - q_25
                q_35 = (
                    np.mod(q_35 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s6_valid:
            ...
            if abs(np.sin(q_521)) < 1e-10:
                ...
                q_36 = q23 - q_26
                q_36 = (
                    np.mod(q_36 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s7_valid:
            ...
            if abs(np.sin(q_522)) < 1e-10:
                ...
                q_37 = q23 - q_27
                q_37 = (
                    np.mod(q_37 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s8_valid:
            ...
            if abs(np.sin(q_522)) < 1e-10:
                ...
                q_38 = q23 - q_28
                q_38 = (
                    np.mod(q_38 - self.qlim3_inf, self.qlim3_sup - self.qlim3_inf)
                    + self.qlim3_inf
                )
        ...
        if s1_valid:
            ...
            q_41 = q234 - q_21 - q_31
            q_41 = (
                np.mod(q_41 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s2_valid:
            ...
            q_42 = q234 - q_22 - q_32
            q_42 = (
                np.mod(q_42 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s3_valid:
            ...
            q_43 = (
                np.mod(q_43 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s4_valid:
            ...
            q_44 = (
                np.mod(q_44 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s5_valid:
            ...
            q_45 = q234 - q_25 - q_35
            q_45 = (
                np.mod(q_45 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s6_valid:
            ...
            q_46 = q234 - q_26 - q_36
            q_46 = (
                np.mod(q_46 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s7_valid:
            ...
            q_47 = (
                np.mod(q_47 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
        ...
        if s8_valid:
            ...
            q_48 = (
                np.mod(q_48 - self.qlim4_inf, self.qlim4_sup - self.qlim4_inf)
                + self.qlim4_inf
            )
