...
class ForwKinematics:
    ...
    def __init__(self, params):
        ...
        self.T_base = (
            transl(self.base_xyz[0], self.base_xyz[1], self.base_xyz[2])
            @ trotx(self.base_rpy[0])
            @ troty(self.base_rpy[1])
            @ trotz(self.base_rpy[2])
        )
        self.T_tool = (
            transl(self.tool_xyz[0], self.tool_xyz[1], self.tool_xyz[2])
            @ trotx(self.tool_rpy[0])
            @ troty(self.tool_rpy[1])
            @ trotz(self.tool_rpy[2])
        )
    ...
    def solve_fk(self, q1, q2, q3, q4, q5, q6):
        ...
        for i in range(len(q)):
            T_i = (
                trotz(q[i] + self.dh_theta_offset[i])
                @ transl(0, 0, self.d[i])
                @ transl(self.a[i], 0, 0)
                @ trotx(self.alpha[i])
            )
