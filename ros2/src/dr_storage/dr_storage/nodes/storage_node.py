...
class StorageNode(Node):
    ...
    def __init__(self):
        ...
        self.save_srv = self.create_service(
            SaveTraj, "/dr/save_traj", self.save_traj_callback
        )
        self.load_srv = self.create_service(
            LoadTraj, "/dr/load_traj", self.load_traj_callback
        )
        self.delete_srv = self.create_service(
            DeleteTraj, "/dr/delete_traj", self.delete_traj_callback
        )
        self.list_srv = self.create_service(
            ListTrajs, "/dr/list_trajs", self.list_trajs_callback
        )
    ...
    def save_traj_callback(self, request, response):
        try:
            ...
            while os.path.exists(
                os.path.join(self.TRAJECTORIES_PATH, f"{name}_v{version}.json")
            ):
                version += 1
            ...
            if not os.path.exists(filepath):
                ...
                if latest < 1:
                    raise FileNotFoundError(
                        f"No se encontró ninguna trayectoria con nombre {name}."
                    )
