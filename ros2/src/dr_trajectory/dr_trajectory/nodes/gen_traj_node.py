...
class GenTrajNode(Node):
    ...
    def __init__(self):
        ...
        self.generator = TrajectoryGenerator(
            params, self.call_ik, self.call_fk, logger=self.get_logger()
        )

        self.srv = self.create_service(
            GenTraj,
            "/dr/gen_traj",
            self.gen_traj_callback,
            callback_group=self.cb_group,
        )

        self._init_timer = self.create_timer(
            0.1, self._init_c0, callback_group=self.cb_group
        )
    ...
    def _init_c0(self):
        ...
        self.get_logger().info(
            f'Estado inicial: q0 = {[f"{v:.3f}" for v in self.q0]}, c0 = {[f"{v:.3f}" for v in self.c0]}'
        )
    ...
    def gen_traj_callback(self, request, response):
        ...
        speed_str = {1: "Lento", 2: "Medio", 3: "Rápido"}.get(
            request.speed_profile, str(request.speed_profile)
        )
        pose_str = [f"{v:.3f}" for v in request.pose]
        self.get_logger().info(
            f"GenTraj solicitado → modo: {modo_str}, speed: {speed_str}, pose: {pose_str}"
        )
        ...
        [success, q_traj, points] = self.generator.generate(
            pose_obj, mode, speed, self.q0, self.c0
        )
